...
// Create a REST API endpoint to generate and return the OAuth 2.0 token
add_action('rest_api_init', function () {
    register_rest_route('firebase/v1', '/get-token/', array(
        'methods' => 'GET',
        'callback' => 'generate_firebase_oauth_token',
        'permission_callback' => '__return_true',
    ));
});

// Function to generate the OAuth 2.0 token using the uploaded credentials
function generate_firebase_oauth_token() {
    if (!file_exists(FIREBASE_JSON_CREDENTIALS_PATH)) {
        return new WP_Error('no_credentials', 'No Google JSON credentials uploaded.', array('status' => 500));
    }
    ...
    try {
        ...
        return rest_ensure_response(array('token' => $token['access_token']));
    } catch (Exception $e) {
        return new WP_Error('token_exception', 'Error generating token: ' . $e->getMessage(), array('status' => 500));
    }
}