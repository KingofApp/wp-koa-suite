<?php
    ...
    function get_push_code(WP_REST_Request $request){
        ...
        if( !apply_filters('determine_current_user', false) ) return new WP_REST_Response( wp_json_encode( array("error" => true, "code" =>"user_not_loged_in")), 400);
            
        $actionStatus = save_user_code($user, $request->get_param('push_code') );
        
        if( !$actionStatus && get_user_meta( $user, 'koa_push_code', true ) != "" ) return new WP_REST_Response( wp_json_encode( array("error" => false, "success" => "code_alredy_set")), 200);
